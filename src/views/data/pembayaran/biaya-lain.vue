			...
			<Table class="mt-3" :keys="{
				...
				<template #bayar="{ item }">
					<form @submit.prevent="
						...
						" class="inline">
						<input type="number" :placeholder="formatMoney(
							item.jumlah_bayar -
							(item.jumlah_bayar * props.siswa.diskon_biaya_lain) / 100 -
							(state.terbayar[item.id] ? state.terbayar[item.id] : 0),
						)
							" min="1" :max="item.jumlah_bayar -
		(item.jumlah_bayar * props.siswa.diskon_biaya_lain) / 100 -
		(state.terbayar[item.id] ? state.terbayar[item.id] : 0)
		" :disabled="item.jumlah_bayar -
		(item.jumlah_bayar * props.siswa.diskon_biaya_lain) / 100 -
		(state.terbayar[item.id] ? state.terbayar[item.id] : 0) <=
		0
		" required
	...
	<Modal v-model="state.modal.save">
		<Card class="w-900px max-w-full" title="Data Pembayaran">
			<div class="mt-4">
					...
					<form @submit.prevent="store" class="rounded shadow w-full" lg="w-1/2">
						<div class="rounded-t p-4 bg-gray-100">
							<div class="form-field">
									...
									<table>
										...
										<tr>
											...
											<td class="text-green-500">
												{{ formatMoney((state.terbayar[state.item.id] || 0) + props.siswa.diskon_biaya_lain / 100 *
													state.item.jumlah_bayar) }}
											</td>
										</tr>
										<tr>
											<td>Kekurangan</td>
								...
								<input type="number" v-model="state.req.jumlah_bayar" placeholder="Masukkan Nominal" min="1"
									:max="state.req.max_jumlah_bayar" :disabled="state.req.max_jumlah_bayar <= 0" required />
							</div>
						</div>
				...
				<div class="bg-red-100 mb-4 border rounded border-red-500 text-red-500 p-3 form-field" v-if="state.isDelete">
					<div class="lg:flex">
